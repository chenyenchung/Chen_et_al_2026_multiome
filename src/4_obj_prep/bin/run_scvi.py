...
parser = argparse.ArgumentParser(
    prog='scverse Runner',
    description='Run scVI on UMI counts'
)

parser.add_argument('-f', '--h5mu', required=True)
parser.add_argument('-o', '--outdir', default='scVI')
...
if args.batch is not None:
    model_dir = os.path.join(args.outdir, f"scVI_{args.batch}_{args.nfactor}")
else:
    model_dir = os.path.join(args.outdir, f"scVI_{args.nfactor}_no_int")

model.save(model_dir, overwrite=True)
latent = model.get_latent_representation()
np.savetxt(os.path.join(args.outdir, f"scVI_{args.nfactor}.csv"), latent, delimiter=',')
...
# Save denoised expression
exp = model.get_normalized_expression(transform_batch='stf_2')
np.savetxt(os.path.join(args.outdir, f"scVI_normexp_{args.nfactor}.csv"), exp, delimiter=',')
